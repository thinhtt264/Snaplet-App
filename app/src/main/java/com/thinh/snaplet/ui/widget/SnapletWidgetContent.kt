...
fun SnapletWidgetContent(
    ...
    when {
        data.isError -> WidgetErrorState(modifier = modifier)
...
private fun WidgetErrorState(
    ...
    Box(
        modifier = modifier.fillMaxSize().background(glanceColor(WidgetStatePanelBackground))
...
private fun WidgetPostState(
    ...
    Box(
        ...
        Box(modifier = GlanceModifier.size(squareEdge)) {
            ...
            // Glance Box has no Modifier.align — use full-size layers + contentAlignment.
            Box(
